...
- **(amended 2026-06-23 per retro `2026-06-22-sec-006-process-gap` proposal #1):** a finding whose original wording no longer accurately describes the system, because a later finding closes the gap it described under a new ID, takes `status: superseded` and records `superseded_by: <new-id>`. A `superseded` finding carries no `expiry:` and does not count toward the medium cap — it documents traceability, it is not an open risk. **`superseded` is not a fourth way to dodge the must-fix rule above.** If the superseded finding was `severity: critical` or `severity: high`, `superseded` is permitted only when either: (a) the `superseded_by:` finding is itself `status: fixed`, or (b) the disposition's `rationale:` field states why the *underlying risk* — not just the finding's wording — is closed, and a second reviewer independently confirms the residual finding's lower severity (recorded with handle + rationale, same as any other downgrade). A `superseded_by:` finding that is `status: accepted`/`status: deferred` at a lower severity, with no such confirmation, does not qualify — the original critical/high stays `open` and blocks RELEASE.
